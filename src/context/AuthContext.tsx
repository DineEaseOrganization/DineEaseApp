...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({children}) => {
    ...
    // Listen to auth events from API client (401 errors)
    useEffect(() => {
    ...
    const refreshUserData = async (): Promise<void> => {
        try {
            ...
            // If refresh fails with 401, the auth event will handle logout
            if (error instanceof ApiError && error.statusCode === 401) {
                // Don't set user to null here - let the auth event emitter handle it
                return;
            }
            throw error;
        }
